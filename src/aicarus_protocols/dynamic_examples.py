...
import json
from .common import (
    EventType, EventTypeRegistry, EventBuilder, 
    event_registry, PROTOCOL_VERSION
)
...
def demo_platform_specific_schemas():
    ...
    discord_schema = {
        "version": "1.0.0",
        "platform": "discord",
        "capabilities": ["text", "embed", "file", "reaction", "thread", "slash_command"],
        "types": {
        ...
        "validation_rules": {
            "allowed_prefixes": ["message.discord", "notice.discord", "request.discord", "action.discord"],
...
def demo_custom_validation_rules():
    ...
    for platform, event_type in test_events:
        ...
        print(f"  {status} {platform} 平台验证 '{event_type}': {'有效' if is_valid else '无效'}")
...
class PlatformEventTypeManager:
    ...
    def validate_and_process_event(self, event: Event) -> bool:
        """验证并处理事件"""
        # 验证事件类型
        if not event_registry.validate_event_for_platform(event.event_type, event.platform):
