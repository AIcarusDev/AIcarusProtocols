...
class Seg:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.type == "seglist" and isinstance(self.data, list):
            ...
            result_data = [
                item.to_dict() if isinstance(item, Seg) else item for item in self.data
            ]
    ...
    def from_dict(cls, data_dict: Dict[str, Any]) -> "Seg":
        ...
        if seg_type == "seglist":
            ...
            if isinstance(raw_seg_data, str):
                ...
                processed_seg_data = (
                    str(raw_seg_data) if raw_seg_data is not None else ""
                )
        else:  # 对于其他所有类型 (如 image, face, notification:*, request:*, action:*)，data 通常是字典
            if isinstance(raw_seg_data, dict):
                processed_seg_data = raw_seg_data
            elif (
                raw_seg_data is None
            ):  # 某些 Seg 的 data 字段可能是可选的，用空字典表示
                processed_seg_data = {}
...
class BaseMessageInfo:
    ...
    anonymity_info: Optional[Dict[str, Any]] = (
        None  # 匿名消息信息, 例如: {"id": Optional[int], "name": str, "flag": str}
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "BaseMessageInfo":
        ...
        return cls(
            ...
            group_info=GroupInfo.from_dict(group_info_data)
            if group_info_data
            else None,
...
if __name__ == "__main__":
    ...
    msg_segments_list = [  # 明确这是一个 Seg 对象的列表
        ...
        Seg(
            type="image",
            data={"url": "http://example.com/image.jpg", "file_id": "qq_image_abc"},
        ),
    ]
    user_message_obj = MessageBase(
        message_info=user_msg_info,
        message_segment=Seg(
            type="seglist", data=msg_segments_list
        ),  # message_segment 是一个类型为 seglist 的 Seg
    ...
    assert reconstructed_user_message.message_info.user_info.additional_data == {
        "constellation": "aries",
        "custom_badge": "vip",
    }
    assert reconstructed_user_message.message_segment.type == "seglist"
    if (
        isinstance(reconstructed_user_message.message_segment.data, list)
        and len(reconstructed_user_message.message_segment.data) > 0
        and isinstance(reconstructed_user_message.message_segment.data[0], Seg)
    ):
    ...
    if (
        isinstance(reconstructed_notification.message_segment.data, list)
        and len(reconstructed_notification.message_segment.data) > 0
        and isinstance(reconstructed_notification.message_segment.data[0], Seg)
        and isinstance(reconstructed_notification.message_segment.data[0].data, dict)
    ):
        assert (
            reconstructed_notification.message_segment.data[0].type
            == "notification:group_file_uploaded"
        )
        assert (
            reconstructed_notification.message_segment.data[0].data["file_info"]["name"]
            == "重要文档.docx"
        )
    else:
    ...
    print(
        "UserInfo with empty additional_data:",
        json.dumps(test_user_info_empty_add.to_dict(), indent=2, ensure_ascii=False),
    )
    ...
    print(
        "BaseMessageInfo with empty additional_config:",
        json.dumps(
            test_msg_info_empty_add_conf.to_dict(), indent=2, ensure_ascii=False
        ),
    )
    ...
    print(
        json.dumps(
            reconstructed_missing_segment.to_dict(), indent=2, ensure_ascii=False
        )
    )
