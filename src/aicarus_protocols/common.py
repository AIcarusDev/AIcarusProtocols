"""
AIcarus-Message-Protocol v1.4.0 - 通用工具和类型定义
提供协议相关的常量、工具函数和构建器。
"""
...
def validate_event_type(event_type: str) -> bool:
    """验证事件类型格式是否正确。"""
    valid_prefixes = ["message.", "notice.", "request.", "action.", "action_response.", "meta."]
...
class EventTypeRegistry:
    ...
    def register_type(self, event_type: str, description: str = "", platform: str = "core") -> bool:
        """
        注册新的事件类型。
    ...
    def register_platform_types(self, platform: str, types_mapping: Dict[str, str]) -> List[str]:
        """
        批量注册平台特定的事件类型。
    ...
    def suggest_event_type(self, platform: str, action: str, context: str = "") -> List[str]:
        """
        基于平台、动作和上下文智能推荐事件类型。
    ...
    def _calculate_type_relevance(self, event_type: str, action: str, context: str) -> float:
        """计算事件类型与动作/上下文的相关性评分。"""
        score = 0.0
        ...
        for keyword, prefixes in action_prefixes.items():
            if keyword in action.lower() and any(prefix in event_parts[0] for prefix in prefixes):
                score += 0.2
    ...
    def _clear_suggestion_cache(self, platform: str = None):
        ...
        if platform:
            # 清理特定平台的缓存
            keys_to_remove = [k for k in self._type_suggestions.keys() if k.startswith(f"{platform}:")]
    ...
    def get_types_by_prefix(self, prefix: str) -> List[str]:
        """获取指定前缀的所有事件类型。"""
        return [event_type for event_type in self._registered_types.keys() 
    ...
    def _validate_by_rules(self, event_type: str, rules: Dict[str, Any]) -> bool:
        ...
        if "allowed_prefixes" in rules:
            return any(event_type.startswith(prefix) for prefix in rules["allowed_prefixes"])
...
class EventType:
    ...
    @classmethod
    def register(cls, event_type: str, description: str = "", platform: str = "core") -> bool:
        """注册新的事件类型。"""
    ...
    def is_valid(cls, event_type: str) -> bool:
        """检查事件类型是否有效。"""
        return event_registry.is_registered(event_type) or validate_event_type(event_type)
    ...
    @classmethod
    def suggest_event_type(cls, platform: str, action: str, context: str = "") -> List[str]:
        """
        基于平台、动作和上下文智能推荐事件类型。
